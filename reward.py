...
def calculate_multi_agent_max():
    ...
    for index in range(min(len(v) for v in average_rewards)):
        if index*AVERAGE_NUM <= LIMIT:
            ...
            queue = []
            for n in range(ACTOR_NUM):
                queue.append(average_rewards[n][index])
            value = max(queue)
            if x_min == 0 and value > 100:
                x_min = index*AVERAGE_NUM
            # if max >= 0:
            #     return_rewards.append(max(queue))
            # else:
            #     return_rewards.append(min(queue))
            return_rewards.append(value)
            return_eps.append((index+1)*AVERAGE_NUM)

    return return_rewards, return_eps, x_min
...
if __name__ == '__main__':
    ...
    multi_average_reward, multi_eps, xmin = calculate_multi_agent_max()
    single_average_reward, single_eps = calculate_single_agent()

    plt.plot(single_eps,single_average_reward, color="#00529a", label="Single DQN")
    plt.plot(multi_eps,multi_average_reward, color="#e41a1c", label="Distibuted DQN")

    #plt.vlines(xmin, -120, 120)
